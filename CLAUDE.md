...
python -m pytest --cov       # coverage (scope/omit in pyproject; CI gates at >=95%)
...
- Coverage scope is `[tool.coverage.run]` in pyproject: the COM/Qt/hardware modules (audio, mixer, display, hdr, gsync, overlay, widgets, tray, main, make_icon, and the caps that wrap them) are omitted — they're verified by running, pyright excludes them. CI enforces `--cov-fail-under=95` on the remaining logic.
